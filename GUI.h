...
nhapPhut:
	...
	while(1){
		...
		if(input == 27){
			...
			cout << input;
			doDaiChuoi++; // do dai so int de phuc vu cho xoa
			time.minute += input - '0';
			time.minute *= 10; 
		}else if(input == 8 && doDaiChuoi > 0){ // backspace
			gotoxy(doDaiChuoi+68, 9);
			cout << " ";	
			doDaiChuoi--;
			gotoxy(doDaiChuoi+69, 9);
			if(time.minute != 0){
				time.minute /= 10;
				if(check && time.minute != 0){
					time.minute /= 10;
					check = false;
				}
			}
			...
			if(doDaiChuoi == 0){
				...
				gotoxy(68, 9);
				cout << string(50, ' ');
				string message = "Thoi gian phai > 0. Moi Nhap lai!";
				MessageBox(0,message.c_str(), "XAC NHAN",0);
				goto nhapPhut;
			} 
			time.minute /= 10;
...
nhapSoCauHoi:
	...
	while(1){
		...
		if(input == 27){
			...
			gotoxy(doDaiChuoi+65, 10);
			cout << " ";	
			doDaiChuoi--;
			gotoxy(doDaiChuoi+66, 10);
			if(soCauHoi != 0){
				soCauHoi /= 10;
				if(check && soCauHoi != 0){
					soCauHoi /= 10;
			...
			if(doDaiChuoi == 0){
				...
				gotoxy(66, 10);
				cout << string(50, ' ');
				string message = "So luong cau hoi phai > 0. Moi Nhap lai!";
				MessageBox(0,message.c_str(), "XAC NHAN",0);
				goto nhapSoCauHoi;
			}
			soCauHoi /= 10;
	...
	layNgauNhienCauHoi(mangCauHoi, soCauHoi, maMonHoc, soLuongCauHoiCuaMotMon, danhSachCauHoi);
	...
	// thoi gian
	int giay = 0, luuTheoGiay1 = 0, luuTheoGiay2 = 0; 
	stop = 1;
	...
	while (stop == 1)
    {
		...
		if(kbhit()){
			...
			}else if( (ch >= 97 && ch <= 100) || (ch >= 65 && ch <=68)){
				gotoxy(60,22);
				setcursor(1,1);
		...
		luuTheoGiay1 = time.second;
		if(luuTheoGiay1+1 == luuTheoGiay2){
			luuSinhVienDangThi(sinhVien->MSV, diemThi);  
		}
		luuTheoGiay2 = luuTheoGiay1;
    }
    clock.join();
...
void tiepTucThi(DSLop dslop, DSMonHoc dsMonHoc, ptrDSCauHoi dsCauHoi, Lop* lop){
	...
	gotoxy(1,5);    	
   	cout << "THOI GIAN| ";
	clock = thread(printClock, ref(time));
	int i = 0, ch, luuTheoGiay1 = 0, luuTheoGiay2 = 0;
	while (stop)
	{	
		...
		if(kbhit()){
			...
			if (ch == 0 || ch == 224){
				...
				gotoxy(40,26);
				cout << "Thoi gian van con! Ban co chac chan muon nop bai (y|Y)? ";
				ch = getch();
				if(ch == 121 || ch == 89){
					gotoxy(40,27);
					cout << "NOP BAI SOM!!!";
